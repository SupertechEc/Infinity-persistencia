...
public abstract class AbstractErrorBuilder<E, B extends AbstractErrorBuilder> {
    ...
    /**
     * <p>
     * Used with enumerated reason code systems. Extracts a message from the
     * enum per</p>
     *
     * <code>error(enumeratedReason.toString())</code>
     *
     * <p>
     * This is used where the business wants to sign off on and control
     * messages, and allows developers to consolidate signed off messages in an
     * enum with it's own Javadoc etc. and manage the messages in source
     * control.</p>
     *
    ...
    /**
     * Creates an exception wrapping the configured response and the
     * <code>cause</code>. The exception implements a Jersey exception type and
     * contains an HTTP response code and body, and therefore any configured
    ...
    /**
     * Creates an exception wrapping the configured response. The exception
     * implements a Jersey exception type and contains an HTTP response code and
     * body, and therefore any configured exception mapping is skipped including
     * {@link RuntimeExceptionMapper}.
     *
     * @return an exception, which should be thrown.
     */
